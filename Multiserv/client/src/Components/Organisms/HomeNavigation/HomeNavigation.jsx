...
const HomeNavigation = () => {
  ...
  const arr = [
    <LinkTo
      ...
      page="home"
      render={<AiFillHome size="28" color="white" />}
    />,
    <LinkTo
      linkClass="m-4 flex justify-center"
      page="home/mapservices"
      render={<IoIosMap size="28" color="white" />}
    />,
    ...
    datosSesionFromLocalStorage ? (
      <LinkTo
        linkClass="m-4 flex justify-center"
        page="profile"
        render={<FaUserAlt size="28" color="white" />}
      />
    ) : null,
    datosSesionFromLocalStorage ? (
      <LinkTo
  ...
  const modal = () => {
    return verPerfil ? (
      <div className={s.UserProfile__OnClick} onMouseLeave={listenOnClick}>
        ...
        {datosSesionFromLocalStorage ? (
          <div
            className="flex items-center justify-center w-2/5 mt-3"
            onMouseOut={listenOnClick}
          >
            <button
              onClick={logout}
              className="font-semibold text-gray-50 flex w-full flex-nowrap bg-green-700 p-2 py-2 px-4 justify-center items-center rounded-md"
            >
              Log out
            </button>
          </div>
