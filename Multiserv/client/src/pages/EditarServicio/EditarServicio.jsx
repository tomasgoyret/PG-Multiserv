...
const EditarServicio = () => {
  ...
  */
  ...
  const [servicio, setServicio] = useState(dataServ)

  /*STATE QUE GUARDA TEMPORALMENTE LOS INPUTS DE EDITAR SERVICIO ANTES DE ACTUALIZARLOS*/
  const [editService, setEditService] = useState(dataServ)
  ...
  const uploadPortada = async () => {
    ...
    try {
      const fileRef = ref(storage, `/PhotosServices/${coverToLoad.name}`);
      ...
      toast.update(notification, {
        render: 'No se pudo actualizer la portada, intenta de nuevo',
        type: 'error',
        isLoading: false,
        position: "top-center",
        autoClose: 3000,
    ...
    <div className="w-full h-screen overflow-y-auto flex flex-col">
      <div css={{ height: '30rem' }} className="w-full relative">
        <img
          css={{ height: '30rem' }}
          alt="portada"
          src={loadedImg ? cover.url : servicio.photos[0]}
