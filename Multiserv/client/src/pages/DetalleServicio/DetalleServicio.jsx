...
const DetalleServicio = () => {
  ...
  const { misFavoritos } = useSelector((state) => state);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const text =
    "El servicio que estabas buscando! Entra y checkea para mas info ";
  ...
  const agregarFav = async () => {
    if (fav.length === 0) {
      ...
      await axios.delete(`eliminar-fav?id=${fav.data.id || fav[0].id}&uidClient=${uid}`);      
      setFav([]);
  ...
  return (
    <div className="w-full">
      <div className="flex flex-col">
        {modalReviews && (
          <div
            className="w-full h-screen z-10 absolute top-0 left-0"
            onClick={handleModalReviews}
          >
            <ModalAllReviews />
          </div>
        )}
          ...
          <div className="w-full flex flex-col overflow-y-auto h-screen pb-10">
            ...
            <div className="flex border-t border-gray-200 my-4 pt-4 w-full">
              <div className="bg-white relative flex flex-col rounded-b-lg w-full">
                ...
                <Image
                  ...
                  imgClass={`object-cover rounded-t-lg w-full h-80 ${
                    loadingImg || failedImg ? "hidden" : ""
                  }`}
                />
                <div className="flex flex-row pt-6 px-4 w-full">
                  ...
                  <div className="relative">
                    ...
                    <div className="flex mt-4">
                      ...
                      {compartirModal && (
                        <div
                          className="w-96 h-48  z-10 absolute -top-5 flex flex-col border border-gray-200 bg-white shadow-xl rounded-lg py-5"
                          onMouseLeave={() => setCompartirModal(false)}
                        >
                          ...
                          <div className="w-full flex justify-center items-center h-3/6">
                            ...
                            >
                              <Image
                                name="whatsappShareServices"
                                imagen={WP}
                                imgClass={`object-cover rounded-t-lg w-10 h-10 rounded-2xl ${
                                  loadingImg || failedImg ? "hidden" : ""
                                }`}
                              />
                            </a>
                            ...
                            >
                              <Image
                                name="twitterShareServices"
                                imagen={TW}
                                imgClass={`object-cover rounded-t-lg w-10 h-10 ${
                                  loadingImg || failedImg ? "hidden" : ""
                                }`}
                              />
                            </a>
                      ...
                      {datosSesionFromLocalStorage &&
                        servicio[0].usuarioUidClient !== uid && (
                          <div className="flex">
                            <button
                        ...
                        servicio[0].usuarioUidClient === uid && (
                          <div className="flex">
                            <button
                              className="flex justify-center mx-2 font-semibold  w-auto text-lg px-4 py-2 bg-green-500 text-gray-50 hover:bg-green-700 active:bg-green-600 rounded-md transition-all ease-in-out duration-300"
                              onClick={() => {
                                return navigate(`/home/${uid}/reservations`);
                              }}
                            >
                              Ver mis turnos
                            </button>
            ...
            <div>
              <ReviewService
                ...
                limitarRenderizadoEnDetalleServicio={true}
              />
            </div>
          </div>
        )}
