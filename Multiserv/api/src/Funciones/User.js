...
// CATEGORÍAS
const Categoriasmockup = async () => {
  try {
    for (const s of Cate) {
      const cat = s.title.charAt(0).toUpperCase() + s.title.slice(1).toLowerCase()
...
const CitasMockup = async () => {
  try {
    ...
    for (let i = 0; i < arrayR.length; i++) {
      const u = await Usuarios.findOne({ where: { uidClient: arrayR[i].usuarioUidClient } })
      const s = await Servicios.findOne({ where: { id: arrayR[i].servicioId } })
      const CitasM = [
